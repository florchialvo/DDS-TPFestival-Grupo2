...
object Auditoria extends Interesado {
  ...
  override def salida(componente: Producto, cantidad: Int) =
    logger.loguear("El producto " + componente.nombre + " bajo del stock minimo")

  override def entrada(componente: Producto, cantidad: Int) =
    logger.loguear("El producto " + componente.nombre + " dejo de estar por debajo del stock minimo")
